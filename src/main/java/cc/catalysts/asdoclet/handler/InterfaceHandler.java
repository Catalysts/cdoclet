...
// --------------------- Interface Handler ---------------------
	...
	public void process(ClassDoc classDoc) {
		...
		if (proxy) {
			cc.catalysts.asdoclet.generator.Type interfaceName = GeneratorUtils.getType(classDoc, getGenerator(), new HashSet<String>());
			...
			cc.catalysts.asdoclet.generator.Type proxyType = GeneratorUtils.getType(proxyName, getGenerator());
			cc.catalysts.asdoclet.generator.Type proxyBaseType = GeneratorUtils.getType(baseType, getGenerator());
	...
	private void processInterfaceInternal(ClassDoc classDoc, Map<String, String> commands) {
		...
		cc.catalysts.asdoclet.generator.Type interfaceName = GeneratorUtils.getType(classDoc, getGenerator(), ignore);
	...
	private void processMethods(ClassDoc classDoc, cc.catalysts.asdoclet.generator.Type classType, boolean bean, Set<String> ignore, Map<String, String> commands, Map<String, cc.catalysts.asdoclet.generator.Type> typeMap) {
	...
	private void processMethod(ClassDoc classDoc, cc.catalysts.asdoclet.generator.Type classType, MethodDoc methodDoc, Set<String> ignore, Map<String, String> commands, Map<String, cc.catalysts.asdoclet.generator.Type> typeMap) {
		...
		cc.catalysts.asdoclet.generator.Type returnType = GeneratorUtils.getType(methodDoc.returnType(), getGenerator(), ignore);
		cc.catalysts.asdoclet.generator.Type methodType = GeneratorUtils.getType(methodDoc, getGenerator(), ignore);
		...
		cc.catalysts.asdoclet.generator.Type actualReturnType = returnType;
		...
		if (!TagParser.getBooleanCommand(Constants.COMMAND_IGNORE, methodCommands)) {
			cc.catalysts.asdoclet.generator.Type asyncType = cc.catalysts.asdoclet.generator.Type.EMPTY;
			...
			if (async) {
				...
				if (callbackType != null) {
					if (callbackType.endsWith("<$type$>")) {
						ArrayList<cc.catalysts.asdoclet.generator.Type> arguments = new ArrayList<cc.catalysts.asdoclet.generator.Type>();
						if (!cc.catalysts.asdoclet.generator.Type.VOID.getQualifiedTypeName().equals(returnType.getQualifiedTypeName())) arguments.add(returnType);

						asyncType = new cc.catalysts.asdoclet.generator.Type(callbackType.substring(0, callbackType.length() - 8), arguments, null, 0, false);
					} else {
						asyncType = new cc.catalysts.asdoclet.generator.Type(callbackType, null, null, 0, false);
					}

					asyncType.setTypeMap(typeMap);
				}

				actualReturnType = methodReturnType != null ? GeneratorUtils.getType(methodReturnType, getGenerator()) : cc.catalysts.asdoclet.generator.Type.VOID;
			}

			boolean isasync = asyncType != cc.catalysts.asdoclet.generator.Type.EMPTY;

			String name = methodDoc.name();
			boolean override = isOverridden(classDoc, name, ignore);
			getGenerator().beginMethod(classType, methodType, 0, actualReturnType, name, isasync, override);
			...
			for (Parameter parameter : methodDoc.parameters()) {
				cc.catalysts.asdoclet.generator.Type type = GeneratorUtils.getType(parameter.type(), getGenerator(), ignore);
	...
	private void traverseInterfaces(ClassDoc interfaceDoc, Set<String> ignore, Map<String, String> parentCommands) {
		for (Type type : interfaceDoc.interfaceTypes()) {
			...
			cc.catalysts.asdoclet.generator.Type interfaceType = GeneratorUtils.getType(type, getGenerator(), ignore);

			if (interfaceType != cc.catalysts.asdoclet.generator.Type.NULL) {
				...
				cc.catalysts.asdoclet.generator.Type interfaceClass = GeneratorUtils.getType(classDoc, getGenerator(), ignore);
				Map<String, cc.catalysts.asdoclet.generator.Type> typeMap = null;

				if (interfaceClass.getArguments() != null && interfaceType.getArguments() != null) {
					typeMap = new HashMap<String, cc.catalysts.asdoclet.generator.Type>();

					Iterator<cc.catalysts.asdoclet.generator.Type> classIterator = interfaceClass.getArguments().iterator();
					Iterator<cc.catalysts.asdoclet.generator.Type> typeIterator = interfaceType.getArguments().iterator();
